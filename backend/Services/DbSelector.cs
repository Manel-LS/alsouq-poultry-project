...
public class DbSelector : IDbSelector
{
    ...
    public Task<IDbConnection> GetConnection(HttpContext httpContext, string? explicitChoice = null)
    {
        ...
        {
            connectionString = $"Server=localhost;Database={choice};Uid=root;Pwd=;SslMode=None;";
        }
        else
        {
            connectionString = _cfg.GetConnectionString("Default") ?? "Server=localhost;Database=SOCERP;Uid=root;Pwd=;SslMode=None;";
        }
