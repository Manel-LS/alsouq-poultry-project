...
{
    /// <summary>
    /// Générateur de rapport de production quotidienne par centre (rap_chair_centre)
    /// </summary>
    public class RapChairCentreReport : BaseReportGenerator, IReportGenerator
    {
            ...
            return Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header()
                        .Column(column =>
                        {
                            column.Item().AlignCenter().Text("تقرير الانتاج اليومي")
                        ...
                        .Column(column =>
                        {
                            ...
                            foreach (var group in groupedData)
                            {
                                ...
                                column.Item().Table(table =>
                                {
                                    ...
                                    {
                                        columns.RelativeColumn(0.7f); // كغ/طير
                                        columns.RelativeColumn(0.9f); // كغ/يوم
                                        columns.RelativeColumn(0.5f); // دليل (poids)
                                        columns.RelativeColumn(0.5f); // الدفعة (poids)
                                        columns.RelativeColumn(1.0f); // عدد الطيور
                                        columns.RelativeColumn(0.5f); // % دليل (mortalité)
                                        columns.RelativeColumn(0.5f); // النسبة (mortalité)
                                        columns.RelativeColumn(0.5f); // العدد (mortalité)
                                        columns.RelativeColumn(0.5f); // اليوم (avant الأسبوع)
                                        columns.RelativeColumn(0.5f); // الأسبوع (après اليوم)
                                        columns.RelativeColumn(0.8f); // رقم الدفعة (avant السلالة)
                                        columns.RelativeColumn(1.0f); // السلالة (après رقم الدفعة)
                                        columns.RelativeColumn(1.0f); // الحظيرة
                                        columns.RelativeColumn(2.0f); // الموقع
                                    });
                                ...
                                // Totaux du groupe - mieux organisés avec espacement
                            ...
                            // Totaux globaux - mieux organisés avec séparateur
        ...
        private void BuildTableHeader(TableDescriptor table)
        {
            table.ColumnsDefinition(columns =>
            {
                ...
                columns.RelativeColumn(0.5f); // دليل (poids)
                columns.RelativeColumn(0.5f); // الدفعة (poids)
                columns.RelativeColumn(1.0f); // عدد الطيور (entre الوزن et النافق)
                columns.RelativeColumn(0.5f); // % دليل (mortalité)
                columns.RelativeColumn(0.5f); // النسبة (mortalité)
                columns.RelativeColumn(0.5f); // العدد (mortalité)
                columns.RelativeColumn(0.5f); // اليوم (avant الأسبوع)
            ...
            // Première ligne d'en-têtes (principaux avec cellules fusionnées) - RTL
            // Ordre: استهلاك الأعلاف, الوزن, عدد الطيور, النافق, يوم/أسبوع, رقم الحركة, السلالة, الحظيرة, الموقع
            table.Header(header =>
            {
        ...
        private void BuildTableSubHeader(TableDescriptor table)
        {
            ...
            // Deuxième ligne d'en-têtes (sous-colonnes) - RTL
            // Ordre: استهلاك الأعلاف, الوزن, عدد الطيور, النافق, يوم/أسبوع, رقم الحركة, السلالة, الحظيرة, الموقع
            ...
            table.Cell().Element(CellHeaderSubStyle).AlignCenter().Text("").FontSize(7); // عدد الطيور
            table.Cell().Element(CellHeaderSubStyle).AlignCenter().Text("% دليل").FontSize(7).Bold().DirectionFromRightToLeft();
            ...
            table.Cell().Element(CellHeaderSubStyle).AlignCenter().Text("").FontSize(7); // رقم الحركة
            table.Cell().Element(CellHeaderSubStyle).AlignCenter().Text("").FontSize(7); // السلالة
            table.Cell().Element(CellHeaderSubStyle).AlignCenter().Text("").FontSize(7); // الحظيرة
            table.Cell().Element(CellHeaderSubStyle).AlignCenter().Text("").FontSize(7); // الموقع
        }
        ...
        private void BuildDataRow(TableDescriptor table, DataRow row)
        {
            ...
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["PoidsGuide"], 0)).FontSize(8).DirectionFromRightToLeft(); // دليل (poids)
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["PoidsLot"], 0)).FontSize(8).DirectionFromRightToLeft(); // الدفعة (poids)
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["effectif"], 0)).FontSize(8).DirectionFromRightToLeft(); // عدد الطيور
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["MortalitePourcentageGuide"], 2)).FontSize(8).DirectionFromRightToLeft(); // % دليل
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["EffectifMvtRatio"], 2)).FontSize(8).DirectionFromRightToLeft(); // النسبة
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["mortalite"], 0)).FontSize(8).DirectionFromRightToLeft(); // العدد
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["jour"], 0)).FontSize(8).DirectionFromRightToLeft(); // اليوم
            table.Cell().Element(CellDataStyle).AlignCenter().Text(FormatNumber(row["semaine"], 0)).FontSize(8).DirectionFromRightToLeft(); // الأسبوع
            table.Cell().Element(CellDataStyle).AlignCenter().Text(row["nummvt"]?.ToString() ?? "").FontSize(8).DirectionFromRightToLeft(); // رقم الحركة
            table.Cell().Element(CellDataStyle).AlignCenter().Text(row["souche"]?.ToString() ?? "").FontSize(8).DirectionFromRightToLeft(); // السلالة
            table.Cell().Element(CellDataStyle).AlignCenter().Text(row["batiment"]?.ToString() ?? "").FontSize(8).DirectionFromRightToLeft(); // الحظيرة
            table.Cell().Element(CellDataStyle).AlignCenter().Text(row["centre"]?.ToString() ?? "").FontSize(8).DirectionFromRightToLeft(); // الموقع
        }
            ...
            double groupPoidsGuide, double groupPoidsLot, double groupConsommationKgJour, double groupConsommationKgPoule, DateTime groupDate)
        {
            ...
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text(FormatNumber(groupPoidsGuide, 0)).FontSize(9).Bold().DirectionFromRightToLeft(); // دليل (poids)
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text(FormatNumber(groupPoidsLot, 0)).FontSize(9).Bold().DirectionFromRightToLeft(); // الدفعة (poids)
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text(FormatNumber(groupEffectif, 0)).FontSize(9).Bold().DirectionFromRightToLeft(); // عدد الطيور
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text("").FontSize(9); // % دليل
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text("").FontSize(9); // النسبة
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text(FormatNumber(groupMortalite, 0)).FontSize(9).Bold().DirectionFromRightToLeft(); // العدد
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text("").FontSize(9); // اليوم
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text("").FontSize(9); // الأسبوع
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text("").FontSize(9); // رقم الحركة
            table.Cell().Element(CellGroupSummaryStyle).AlignRight().Text($" :المجموع").FontSize(9).Bold().DirectionFromRightToLeft(); // السلالة
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text("").FontSize(9); // الحظيرة
            table.Cell().Element(CellGroupSummaryStyle).AlignCenter().Text("").FontSize(9); // الموقع
        }
            ...
            double totalPoidsGuide, double totalPoidsLot, double totalConsommationKgJour, double totalConsommationKgPoule)
        {
            ...
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text(FormatNumber(totalPoidsGuide, 0)).FontSize(10).Bold().DirectionFromRightToLeft(); // دليل (poids)
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text(FormatNumber(totalPoidsLot, 0)).FontSize(10).Bold().DirectionFromRightToLeft(); // الدفعة (poids)
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text(FormatNumber(totalEffectif, 0)).FontSize(10).Bold().DirectionFromRightToLeft(); // عدد الطيور
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text("").FontSize(10); // % دليل
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text("").FontSize(10); // النسبة
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text(FormatNumber(totalMortalite, 0)).FontSize(10).Bold().DirectionFromRightToLeft(); // العدد
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text("").FontSize(10); // اليوم
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text("").FontSize(10); // الأسبوع
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text("").FontSize(10); // رقم الحركة
            table.Cell().Element(CellSummaryStyle).AlignRight().Text(":المجموع العام ").FontSize(10).Bold().DirectionFromRightToLeft(); // السلالة
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text("").FontSize(10); // الحظيرة
            table.Cell().Element(CellSummaryStyle).AlignCenter().Text("").FontSize(10); // الموقع
        }
    }
}
