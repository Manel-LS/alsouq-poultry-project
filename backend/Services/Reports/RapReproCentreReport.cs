...
{
    /// <summary>
    /// Générateur de rapport de production quotidienne par centre (rap_repro_centre)
    /// </summary>
    public class RapReproCentreReport : BaseReportGenerator, IReportGenerator
    {
        public byte[] GenerateWithGrouping(string connectionString, string codeUser, string societe)
        {
            ...
            return Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    // En-tête du rapport (RTL) - Repro
                    page.Header()
                        .Column(column =>
                        {
                            column.Item().AlignCenter().Text("تقرير الانتاج اليومي - poule repro")
