    ...
    public class RapportJournalierController : ControllerBase
    {
        ...
        public async Task<IActionResult> ValiderJournee([FromBody] ValidationJourneeRequest request)
        {
            ...
            return await strategy.ExecuteAsync(async () =>
            {
                ...
                var nomBase = request.NomBaseStockSession?.ToString() ?? string.Empty;
...
#pragma warning restore EF1002
        ...
        private async Task<NumerosMouvements> GetNumerosMouvementsExistants(ValidationJourneeRequest request)
        {
            ...
            numeros.NumCons = mouvementConsommation?.nummvt ?? string.Empty;
            ...
            numeros.NumProd = mouvementProduction?.nummvt ?? string.Empty;
        ...
        private Task<List<MouvementItem>> RecupererMouvements(ValidationJourneeRequest request)
        {
            ...
            return Task.FromResult(mouvements.OrderBy(m => m.NLigne).ToList());
        }
        ...
        private async Task<bool> RemplirLigne(MouvementItem mouvement, ArticleDetail article, string numMvt, string typeMvt, string groupage, ValidationJourneeRequest request, TotauxJournee totaux, Dictionary<string, double> prochainesLignesParMouvement)
        {
            ...
            {
                qteLiv = await CalculerQuantiteLivree(groupage, article.Famille, article.Code, request.Date, codeDepBat ?? string.Empty);
            }
            ...
            if (typeMvt == "E")
            {
                await CreationLot(codeDepBat ?? string.Empty, article.Code, article.Famille, groupage,
        ...
        private Task CreationLot(string? codeDep, string codeArt, string famille, string numLot, DateTime dateExp, string libelle, double prix, DateTime date, int alerteExp)
        {
